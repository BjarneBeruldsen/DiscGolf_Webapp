    ...
    return (
        <div className="fixed top-16 right-4 bg-gray-200 text-black rounded-lg shadow-md p-4 z-50 w-80">
            <div className="flex justify-between items-center border-b pb-2">
                <h1 className="text-md font-bold">{t('Oppdateringer')}</h1>
                <button className="text-2xl hover:bg-gray-500 hover:rounded-full px-3 pb-1" onClick={toggleVarsling}>x</button>
            </div>
            {error && <div>{error}</div>}
            {isPending && laster && <div>{t('Laster...')}</div>}
            <p className="text-xs mt-2">{t('Her er dine siste varsler.')}</p>
            ...
            {varsling.nyhetTittel ? (
                ...
                <>
                    <h2 className="text-md font-bold">{t('Invitasjon til runde')}</h2>
                    <p className="text-sm">{t('Avsender')}: {varsling.invitasjon.avsender || t('Ikke definert')}</p>
                    <div className="">
                ...
                <p className="text-sm">{t('Ukjent varslingstype')}</p>
            )}
        </div>
    ))
) : (
    <p className="text-xs">{t('Ingen varsler tilgjengelig.')}</p>
)}
